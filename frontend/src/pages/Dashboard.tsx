  ...
  return (
    <div className="dashboard-page">
      <SectionHeader
        ...
        description="Sözleşmelerinizi ve yapay zekâ destekli hukuki incelemelerinizi tek merkezden yönetin."
      ...
      <div className="dashboard-grid">
        ...
        <StatCard icon="clock" label="İşlem Bekleyen Belgeler" value={dashboard.metrics.pending} helper="Belge işleme devam ediyor" tone="warning" />
        <StatCard icon="alert" label="Başarısız İşlemler" value={dashboard.metrics.failed || "—"} helper="Yeniden işleme gerektiren" tone="danger" />
        <StatCard icon="shield" label="Mevzuat Uyumu Ortalaması" value="—" helper="Raporlar sonrası hesaplanır" />
      </div>

      <div className="dashboard-layout">
        <Card className="dashboard-primary-panel">
          <div className="panel-heading"><div><span>SON SÖZLEŞMELER</span><h2>Sözleşmeler</h2></div><Link to="/upload">Yeni belge yükle <Icon name="arrow" size={15} /></Link></div>
            ...
            <div className="contract-table" role="table" aria-label="Son sözleşmeler">
              ...
              {recentContracts.map((contract) => (
                <Link className="contract-table-row" key={contract.id} to={`/contracts/${contract.id}`} role="row">
                  ...
                  <span><Badge tone={statusTone(contract.status)}>{contractStatusLabels[contract.status] ?? contract.status}</Badge></span>
        ...
        <div className="dashboard-side-column">
          <Card className="quick-actions"><div className="panel-heading"><div><span>HIZLI İŞLEMLER</span><h2>Başlayın</h2></div></div><div className="quick-action-grid">
            ...
            <Link to="/chat"><Icon name="sparkle" /><span><strong>Hukuk asistanına danış</strong><small>Kaynaklandırılmış cevap</small></span></Link>
            <Link to="/compare"><Icon name="compare" /><span><strong>Karşılaştır</strong><small>İki sürümü incele</small></span></Link>
            {user?.is_admin && <Link to="/legal-kb"><Icon name="book" /><span><strong>Hukuk Bilgi Tabanı</strong><small>Kaynakları yönet</small></span></Link>}
          </div></Card>
          <Card className="recent-chats"><div className="panel-heading"><div><span>SON SOHBETLER</span><h2>Hukuk Asistanı Oturumları</h2></div><Link to="/chat">Aç</Link></div>
            {recentConversations.length ? recentConversations.map((conversation) => <Link to={`/chat?conversation=${conversation.id}`} key={conversation.id}><i><Icon name="chat" size={16} /></i><span><strong>{conversation.title}</strong><small>{formatDate(conversation.updated_at)}</small></span></Link>) : <EmptyState icon="chat" title="Sohbet bulunmuyor" description="Hukuk asistanıyla yeni bir oturum başlatın." />}
          </Card>
        </div>
      </div>

      <Card className="recent-activity"><div className="panel-heading"><div><span>SON HAREKETLER</span><h2>Belge işlemleri</h2></div></div><div className="activity-list">{recentContracts.slice(0, 3).map((contract) => <div key={contract.id}><i><Icon name="activity" size={15} /></i><p><strong>{contract.original_filename}</strong> için işlem durumu <Badge tone={statusTone(contract.status)}>{contractStatusLabels[contract.status] ?? contract.status}</Badge> olarak güncellendi.</p><time>{formatDate(contract.updated_at)}</time></div>)}</div></Card>
    </div>
  );
}
